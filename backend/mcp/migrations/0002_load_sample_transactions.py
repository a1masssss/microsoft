...
def load_sample_data(apps, schema_editor):
    ...
    if Transaction.objects.exists():
        print(f"⚠️  Transactions already exist ({Transaction.objects.count():,} records). Skipping data load.")
        return
    ...
    if not parquet_file.exists():
        print(f"⚠️  Sample data file not found: {parquet_file}")
        print("   Run: python manage.py load_transactions --limit 100000")
        return
    
    print(f"📦 Loading sample transaction data from {parquet_file.name}...")
    ...
    print(f"✅ Successfully loaded {inserted_count:,} transaction records!")
